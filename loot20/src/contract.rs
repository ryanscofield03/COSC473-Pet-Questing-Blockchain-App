...
pub fn instantiate(
    ...
    Ok(Response::default()
        .add_attribute("admin_chosen", admin)
        .add_attribute("admin_saved", CONFIG.load(deps.storage)?.admin)
    )
}
...
fn check_if_admin(config_admin: &Addr, account: &Addr) -> StdResult<()> {
    if config_admin != account {
        ...
        return Err(StdError::generic_err(
            format!("LOOT: This is an admin command. Admin commands can only be run from admin address. \
            admin account {config_admin}, other account {account}"),
        ));
    }

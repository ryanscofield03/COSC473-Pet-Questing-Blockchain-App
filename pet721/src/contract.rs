...
pub fn instantiate(
    ...
    let minters = vec![admin_raw.clone()];
    ...
    let admin_chosen = deps.api.addr_humanize(&admin_raw)?.to_string();
    let admin_saved = deps.api.addr_humanize(&load::<Config>(deps.storage, CONFIG_KEY)?.admin)?.to_string();
    Ok(Response::new()
        .add_messages(messages)
        .add_attribute("admin_chosen", admin_chosen)
        .add_attribute("admin_saved", admin_saved)
    )
}
...
pub fn add_minters(
    ...
    check_status(config.status, priority)?;
    let sender_raw = deps.api.addr_canonicalize(sender.as_str())?;
    if config.clone().admin != sender_raw {
        let config_admin = config.clone().admin;
        return Err(StdError::generic_err(
            format!("PET: This is an admin command. Admin commands can only be run from admin address. \
            admin account {config_admin}, other account {sender_raw}"),
        ));
    }
